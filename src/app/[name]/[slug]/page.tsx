...
type PageDataProps = { 
  pageData: { data: Data[], icon?: { name: keyof typeof dynamicIconImports, color?: string, size?: string }, image?: { src: string, alt: string } };
  mdContent: string;
};

export default async function Servei({ params }: { params: { name: string, slug: string } }) {
    const { name, slug } = params;
    const { pageData, mdContent }: PageDataProps = await getData({ name, slug });
...
const getData = async ({ name, slug }: { name: string, slug: string}): Promise<any> => {
  const [pageData, mdContent] = await Promise.all([
