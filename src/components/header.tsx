  ...
  const iconRoutes = routes.find((r) => r.icon);
    return (
        <header className='px-4 lg:px-6 py-2 md:h-20 flex flex-col md:flex-row items-center border-b'>
            ...
            <nav className='w-full sm:max-md:max-w-[320px] md:max-w-xl uppercase overflow-x-auto my-2 md:my-0 md:ml-auto md:mr-4 flex items-center justify-between gap-4 sm:gap-6'>
                {textRoutes.map(
                    (r, index): JSX.Element => (
                        <Link
                            key={index + r.name}
